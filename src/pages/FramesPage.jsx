    ...
    <div className="space-y-8 animate-fade-in">
      <div className="flex items-start justify-between">
        <div>
          ...
          <p className="text-gray-500 font-medium mt-1">Organized automatically based on file name</p>
        </div>
      ...
      {frames.length === 0 ? (
        <div className="glass-card p-12 text-center">
          ...
          <p className="text-sm text-gray-600 mt-2">Upload transparent PNG frames. Include 'strip-4' or 'grid-6' in the name to auto-organize!</p>
        </div>
      ) : (
          ...
          {groupedFrames.map(group => (
            <div key={group.id} className="space-y-4 animate-fade-in">
              <div className="flex items-center gap-3 border-b border-white/[0.06] pb-3">
                ...
                <span className="px-2.5 py-0.5 rounded-full bg-white/10 text-[11px] font-bold text-gray-400">{group.items.length}</span>
              </div>
              ...
              <div className="grid grid-cols-2 md:grid-cols-3 xl:grid-cols-4 gap-5 stagger-children">
                {group.items.map((frame) => (
                  <div 
                    key={frame.id} 
                    className={`glass-card overflow-hidden group transition-all duration-300 ${!frame.is_active ? 'opacity-50' : ''}`}
                  >
                    ...
                    <div className="p-4 space-y-3">
                      <div>
                        ...
                        <p className="text-[10px] text-gray-600 mt-0.5">{new Date(frame.created_at).toLocaleDateString()}</p>
                      </div>
                      <div className="flex items-center justify-between">
                        <button 
                          ...
                          className={`flex items-center gap-1.5 text-[10px] font-bold uppercase tracking-wider px-3 py-1.5 rounded-lg transition-colors ${
                            frame.is_active ? 'text-emerald-400 bg-emerald-500/10 hover:bg-emerald-500/20' : 'text-gray-500 bg-white/[0.04] hover:bg-white/[0.08]'
                          }`}
                        >
                          {frame.is_active ? <ToggleRight className="w-3.5 h-3.5" /> : <ToggleLeft className="w-3.5 h-3.5" />}
                          {frame.is_active ? 'Active' : 'Inactive'}
                        </button>
                        <button onClick={() => deleteFrame(frame)} className="text-gray-600 hover:text-red-400 transition-colors p-1.5 rounded-lg hover:bg-red-500/10" title="Delete frame">
